...
def admin_price_master():
    ...
    # Show current price master table
    if rows:
        ...
        st.subheader("Current Price Master")
        st.dataframe(
            df.style.format({"Default Each Time Cost": "${:,.2f}".format}),
            use_container_width=True,
        )

        st.markdown("### ✏️ Edit / Delete a Selected Entry")

        # Build selection of existing rows
        options = {
            f"{r['category']} — {r['frequency']} (ID {r['id']})": r for r in rows
        }
        ...
        with st.form("pm_edit_form_single"):
            ...
            with col1:
                ...
                edit_frequency = st.text_input(
                    "Frequency",
                    value=selected_row["frequency"],
                    key="pm_freq_selected",
                )
            with col2:
                edit_default_cost = st.number_input(
                    "Default Each Time Cost (USD)",
                    min_value=0.0,
                    max_value=100000.0,
                    value=float(selected_row["default_cost"] or 0.0),
                    step=1.0,
                    key="pm_cost_selected",
                )
            edit_notes = st.text_input(
                "Notes (optional)",
                value=selected_row.get("notes") or "",
                key="pm_notes_selected",
            )
        ...
        if save_btn:
            if not edit_category or not edit_frequency:
                st.error("Category and Frequency are required.")
            else:
                db.update_price_master_entry(
                    entry_id=rid,
                    category=edit_category.strip(),
                    frequency=edit_frequency.strip(),
                    default_cost=float(edit_default_cost),
                    notes=edit_notes.strip(),
                )
                st.success("Price master entry updated.")
                st.rerun()

        if delete_btn:
            db.delete_price_master_entry(rid)
            st.warning("Price master entry deleted.")
            st.rerun()
    else:
        st.info("No price master entries yet. Use the form below to add the first one.")
...
def admin_event_scheduler(user: Dict[str, Any]) -> None:
    ...
    with st.form("schedule_activity_form"):
        ...
        with col_p2:
            service_options: Dict[str, Dict[str, Any]] = {}
            for s in services:
        ...
        provider_option_map: Dict[str, Optional[int]] = {"(Unassigned)": None}
        ...
        # Date, time & follow-up details
        today = date.today()
        scheduled_date = st.date_input("Scheduled Date", value=today)
        scheduled_time = st.time_input("Scheduled Time", key="evt_time")
    ...
    with col_f2:
        default_to = today
        try:
            default_to = today.replace(day=min(today.day + 7, 28))
        except Exception:
            pass
        to_date = st.date_input("To date", value=default_to, key="evt_to")
    ...
    st.markdown("---")
    st.markdown("### 🔁 Manage Individual Activities, Edit, Reminders & Follow-ups")

    # Build simple lookup maps for reuse in edit forms
    prop_lookup = {p["id"]: p for p in properties}
    sp_lookup = {sp["id"]: sp for sp in service_persons}

    for e in events:
        header = f"[# {e['id']}] {e['scheduled_date']}"
        if e.get("scheduled_time"):
            header += f" {e['scheduled_time']}"
        header += f" — {e['property_name']} — {e['service_category']}"
        with st.expander(header):
            st.write(f"**Property:** {e['property_name']}")
            st.write(f"**Service / Activity:** {e['service_category']}")
            st.write(f"**Scheduled Date:** {e['scheduled_date']} {e.get('scheduled_time') or ''}")
            st.write(f"**Current Status:** {e['status']} ({e['due_state']})")

            # --- Edit core event details (property, service, provider, date & time, category) ---
            st.markdown("#### ✏️ Edit Event Details")
            with st.form(f"evt_edit_form_{e['id']}"):
                # Property selection
                col_ep1, col_ep2 = st.columns(2)
                with col_ep1:
                    prop_option_map2 = {f"{p['name']} (ID {p['id']})": p["id"] for p in properties}
                    # Preselect current property
                    current_prop_label = None
                    for lbl, pid in prop_option_map2.items():
                        if pid == e["property_id"]:
                            current_prop_label = lbl
                            break
                    sel_prop_label2 = st.selectbox("Property", list(prop_option_map2.keys()), index=list(prop_option_map2.keys()).index(current_prop_label) if current_prop_label else 0, key=f"evt_prop_{e['id']}")
                    edit_property_id = prop_option_map2[sel_prop_label2]

                    # Load services for chosen property
                    services2 = db.get_services_for_property(edit_property_id)
                    services2 = [dict(s) for s in services2]

                with col_ep2:
                    service_options2: Dict[str, Dict[str, Any]] = {}
                    for s in services2:
                        label2 = f"{s['category']} — {s['frequency']} (service #{s['id']})"
                        service_options2[label2] = s
                    service_labels2 = list(service_options2.keys())
                    service_labels2.append("Ad-hoc / Custom activity")

                    current_service_label = None
                    if e.get("service_id") is not None:
                        for lbl2, s2 in service_options2.items():
                            if s2["id"] == e["service_id"]:
                                current_service_label = lbl2
                                break
                    default_index2 = service_labels2.index(current_service_label) if current_service_label in service_labels2 else len(service_labels2) - 1
                    sel_service_label2 = st.selectbox(
                        "Service / Activity",
                        service_labels2,
                ...
                # Determine category & service_id for edit
                edit_service_id = None
                edit_category = e["service_category"]
                if sel_service_label2 == "Ad-hoc / Custom activity":
                    edit_category = st.text_input("Custom Category / Activity Name", value=edit_category, key=f"evt_cat_{e['id']}")
                else:
                    chosen2 = service_options2[sel_service_label2]
                    edit_service_id = int(chosen2["id"])
                    edit_category = str(chosen2["category"])

                col_dt1, col_dt2 = st.columns(2)
                with col_dt1:
                    # Date
                    try:
                        cur_date = date.fromisoformat(e["scheduled_date"])
                    except Exception:
                        cur_date = today
                    edit_date = st.date_input("Scheduled Date", value=cur_date, key=f"evt_date_{e['id']}")
                with col_dt2:
                    # Time
                    ...
                    if e.get("scheduled_time"):
                        try:
                            hh, mm = e["scheduled_time"].split(":")
                            cur_time_obj = _time(int(hh), int(mm))
                        except Exception:
                            cur_time_obj = None
                    if cur_time_obj is None:
                        cur_time_obj = _time(9, 0)
                    edit_time = st.time_input("Scheduled Time", value=cur_time_obj, key=f"evt_time_{e['id']}")

                # Provider
                provider_option_map2: Dict[str, Optional[int]] = {"(Unassigned)": None}
                current_provider_label2 = "(Unassigned)"
                for sp in service_persons:
                    if not sp.get("is_active", 1):
                        continue
                    lbl_sp = f"{sp['full_name']} (ID {sp['id']})"
                    provider_option_map2[lbl_sp] = sp["id"]
                    if e.get("provider_id") == sp["id"]:
                        current_provider_label2 = lbl_sp

                sel_provider_label2 = st.selectbox(
                    "Assign Service Person (optional)",
                    list(provider_option_map2.keys()),
                    index=list(provider_option_map2.keys()).index(current_provider_label2),
                    key=f"evt_prov_{e['id']}",
                )
                edit_provider_id = provider_option_map2[sel_provider_label2]

                save_core = st.form_submit_button("Save Event Details")

            if save_core:
                db.update_service_event_core(
                    event_id=e["id"],
                    property_id=edit_property_id,
                    service_id=edit_service_id,
                    provider_id=edit_provider_id,
                    service_category=edit_category,
                    scheduled_date=edit_date.isoformat(),
                    scheduled_time=edit_time.strftime("%H:%M") if edit_time else None,
                )
                st.success("Event details updated.")
                st.rerun()

            # Status & follow-up form
            st.markdown("#### ✅ Status & Follow-up")
            ...
            if e.get("provider_id") and (e.get("provider_email") or e.get("provider_phone")):
                ...
                if st.button(
                    ...
                    body = (
                        ...
                        f"- Scheduled date: {e['scheduled_date']} {e.get('scheduled_time') or ''}\n\n"
